...
import StatusBadge, { getCardStatusBadge } from '@/components/admin/StatusBadge'

interface CardWithProfile extends Omit<SessionCard, 'profile'> {
  profile: { full_name: string | null; id: string; email?: string | null } | null
}
...
export default function CardsClient({
  ...
  const filteredClients = allFilteredClients.slice(0, 50)
  ...
  const filters: Filter[] = [
    ...
    },
    {
      ...
      placeholder: 'Tous',
      options: [
        { value: '1', label: '1 séance' },
        { value: '10', label: '10 séances' },
        { value: '20', label: '20 séances' }
      ]
    }
  ]
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-stone-500 text-sm">{totalCount} carte{totalCount > 1 ? 's' : ''} au total</p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {cards.length === 0 ? (
                <tr>
                  <td colSpan={6} className="text-center text-stone-400 py-8">Aucune carte.</td>
                </tr>
              ) : (
                cards.map((card) => {
                  const statusBadge = getCardStatusBadge(card.remaining_sessions, card.expiry_date)
                  return (
